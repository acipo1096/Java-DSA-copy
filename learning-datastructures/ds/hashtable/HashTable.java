...
public class HashTable {
    ...
    // Double Hashing - we need TWO hash functions
    // The first will give us the index position where we'd like to insert
    // The second will give us the step size in case the index function where we'd like to insert is already taken
    ...
    private int hashFunc1(String word){
        ...
        return hashVal; // ideal index position we'd like to insert into or search
    }

    // returns step size
    private int hashFunc2(String word){
        int hashVal = word.hashCode();
        hashVal %= arraySize; // could also be written as hashVal = hashVal % arraySize
        if (hashVal <= 0) {
            hashVal += arraySize;
        }
        ...
        // Can be any prime number; went with 3 for simplicity's sake - but it must be smaller than arraySize
        return 3 - hashVal % 3;
    }
